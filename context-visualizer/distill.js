...
function buildEndpoint() {
  return (LLM_BASE.endsWith("/chat/completions") ? LLM_BASE.replace(/\/chat\/completions$/, "") : LLM_BASE)
       .replace(/\/+$/, "");
}

async function callModel(messages, opts) {
  const baseUrl = buildEndpoint();
  const chatUrl = baseUrl + "/chat/completions";
  const res = await fetch(chatUrl, { method: "POST", headers: {
    "Authorization": "Bearer " + OR_KEY,
    "Content-Type": "application/json",
    "HTTP-Referer": "http://localhost",
    "X-Title": "conv-viz-storyline"
  }, body: Core.reqBody(MODEL, messages, opts, useJsonSchema) });
...
module.exports = { distillSessions, callModel, buildEndpoint, MODEL, CONCURRENCY, BLOCK_SIZE, MAX_PER_AGENT };
