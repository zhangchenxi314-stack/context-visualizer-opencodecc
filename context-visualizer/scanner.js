...
async function opencodeExport(sessionId) {
  return new Promise((resolve, reject) => {
    const tmp = path.join(os.tmpdir(), `oc-export-${sessionId.slice(-8)}-${Date.now()}.json`);
    exec(
      `opencode export "${sessionId}" > "${tmp}" 2>/dev/null`,
      { timeout: 60000 },
      (err) => {
        if (err) {
        ...
        try {
          ...
          reject(new Error(`opencode export read failed: ${e.message}`));
        }
      }
    );
  });
...
async function opencodeExportSync(sessionId) {
  const tmp = path.join(os.tmpdir(), `oc-export-${sessionId.slice(-8)}-${Date.now()}.json`);
  try {
    execSync(`opencode export "${sessionId}" > "${tmp}" 2>/dev/null`, { timeout: 60000 });
    const text = fs.readFileSync(tmp, "utf8");
    fs.unlinkSync(tmp);
    const data = safeJson(text);
