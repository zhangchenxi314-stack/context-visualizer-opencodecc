...
// Timestamp normalization: accepts ISO string OR millisecond number (OpenCode format).
...
// ---------- model plumbing ----------
// build the request body for OpenAI-compatible /v1/chat/completions.
// opts.extraBody: provider-specific fields to merge (e.g. {reasoning:{enabled:false}} for OpenRouter).
// opts.useJsonSchema: use json_schema (strict) instead of json_object.
// opts.forceJsonObject: override to json_object regardless of useJsonSchema.
function reqBody(model, messages, opts, useJsonSchema) {
  opts = opts || {};
  const b = { model, max_tokens: opts.maxTok || 500, messages };
  if (opts.extraBody) Object.assign(b, opts.extraBody);
  if (opts.noResponseFormat) {
    // Skip response_format for providers that don't support it (DeepSeek, internal APIs, etc.)
    // The system prompt already tells the model to output JSON.
  } else if (useJsonSchema && !opts.forceJsonObject) b.response_format = { type: "json_schema", json_schema: { name: "event", strict: true, schema: {
...
return { // public API
  ...
  turnFromLine, turnsFromText, preFilter, reqBody,
