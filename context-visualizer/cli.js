...
async function showPreviewAndDistill(session, platform, config) {
  ...
  if (platform === "opencode") {
    try {
      ...
      console.log(`  无法加载会话详情: ${e.message}`);
      console.log("  将使用已知元数据进行预览");
    }
  }
  ...
  let useLLM = hasLLM;
  if (hasLLM) {
    console.log(`  检测到 LLM 配置 (${config.llm?.model || "default"}@${config.llm?.baseUrl || "openrouter"})`);
  ...
  try {
    ...
    console.log(`  审计发现 ${(e.stderr || "").split("\n").filter(l => l.includes("violation")).join(" ")}`);
  }
