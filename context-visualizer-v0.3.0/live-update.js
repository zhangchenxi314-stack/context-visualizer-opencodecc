...
function userTurns(file){const L=fs.readFileSync(file,"utf8").split("\n");const t=[];
  L.forEach((l,i)=>{if(!l.trim())return;let o;try{o=JSON.parse(l)}catch(e){return;}
    if(o.type==="queue-operation"&&o.operation==="enqueue")t.push({line:i+1,ts:o.timestamp||"",text:clean(o.content)});
    else if(o.type==="user"&&o.message&&o.message.role==="user"){
      const text=clean(typeof o.message.content==="string"?o.message.content:Array.isArray(o.message.content)?o.message.content.filter(b=>b&&b.type==="text").map(b=>b.text||"").join("\n"):"");
